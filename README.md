...
└── assets/
    ...
    ├── Hammad-Ali-Odoo-ERP-Developer-CV.pdf   # designed CV (2pp)
    ├── Hammad-Ali-CV-ATS.pdf                # single-column ATS version
    ├── cv_source.html                       # source of the designed CV
    ├── cv_ats_source.html                   # generated by build_ats_cv.py
    └── images/
...
### 23 July 2026 (second pass)

- Added the four TTI Testing Laboratories projects: employee self-service portal,
  attendance dashboard, field sales module, loan management. All Odoo 18
  Enterprise; the first three use OWL
- **OWL restored to the skills list** — now backed by three shipped Odoo 18
  Enterprise modules. Odoo.sh, OAuth 2.0 and JSON-RPC remain out, still unevidenced
- Stats now 14 client / 19 built, led by Odoo version breadth rather than tenure
- Featured trimmed to 7 (was heading for 8); the home grid shows the newest 3
- `assets/cv_source.html` rebuilt: TTI projects added, the two Aetos roles merged
  into one 16-month tenure with a visible promotion, results band added under the
  summary, Odoo.sh / OAuth / JSON-RPC removed, degree line shortened,
  PDF `/Title` fixed (was `cv.html`). Now **2 pages**, was 3
- New `Hammad-Ali-Odoo-ERP-Developer-CV.pdf` (replaces `Developer_hammad.pdf`)
- **New ATS-safe CV** — `build_ats_cv.py` generates `assets/cv_ats_source.html`
  from `portfolio_data.yaml`, printed to `assets/Hammad-Ali-CV-ATS.pdf`.
  The designed CV's flex date rows detach dates from employers in the PDF text
  layer (the education date extracted 30 lines from the degree). The ATS version
  is strictly single-column with dates inline. Use it for job boards; use the
  designed one for the portfolio download and direct email
- 4 `TO CONFIRM` blocks added flagging that Employee Portal and attendance are
  now claimed by three separate projects (ClearPath, Axis, TTI)

### 23 July 2026 (first pass)
